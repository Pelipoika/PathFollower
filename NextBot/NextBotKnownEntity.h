...
public:
	...
	virtual void UpdatePosition( void )		// could be seen or heard, but now the entity's position is known
	{
		if ( m_who.Get() )
		{
			...
			auto goal_bcc = static_cast<CBaseCombatCharacter *>(m_who.Get());

			m_lastKnownArea = goal_bcc->MyCombatCharacterPointer() ? goal_bcc->MyCombatCharacterPointer()->GetLastKnownArea() : NULL;
