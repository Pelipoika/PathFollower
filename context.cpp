...
public:
	...
	float operator()(CNavArea *area, CNavArea *fromArea, const CNavLadder *ladder = nullptr, const CFuncElevator *elevator = nullptr, float length = -1.0f) const override
	{
		...
		const float delta_z = fromArea->ComputeAdjacentConnectionHeightChange(area);
